...
if (mysqli_num_rows($sql_buku) > 0) {
    ...
    $data_buku = mysqli_fetch_assoc($sql_buku);
    ...
    if ($status_upload == 1) {
        ...
        if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
            ...
            $query  = "
                UPDATE buku 
                SET id_user='$id', id_kategori='$id_kategori', judul='$judul', slug='$slug', sinopsis='$sinopsis', 
                isi='$isi', tags='$tags', cover='$name_file_db', status='$status'
                WHERE id='$id_buku'
            ";
            $sql    = mysqli_query($koneksi, $query) or die(mysqli_error($koneksi));
            if ($sql) {
                header("location: ../../public/index.php?page=profile&sub_page=buku-ku&info=Berhasil Mengubah buku");
            } else {
                header("location: ../../public/index.php?page=tambah-buku&info=Gagal Mengubah buku");
            }
        ...
        $query  = "
                UPDATE buku 
                SET id_user='$id', id_kategori='$id_kategori', judul='$judul', slug='$slug', sinopsis='$sinopsis', 
                isi='$isi', tags='$tags', status='$status'
                WHERE id='$id_buku'
            ";
        $sql    = mysqli_query($koneksi, $query) or die(mysqli_error($koneksi));
        if ($sql) {
